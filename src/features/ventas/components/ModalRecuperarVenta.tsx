...
export default function ModalRecuperarVenta({ cajaId, onCerrar, onRecuperada }: Props) {
  ...
  function manejarTecla(e: React.KeyboardEvent) {
    ...
    if (e.key === "ArrowDown") { e.preventDefault(); setIndiceSeleccionado((i) => Math.min(i + 1, suspendidas.length - 1)); return; }
    if (e.key === "ArrowUp") { e.preventDefault(); setIndiceSeleccionado((i) => Math.max(i - 1, 0)); return; }
    if (e.key === "Enter" && suspendidas[indiceSeleccionado]) { recuperar(suspendidas[indiceSeleccionado]); }
  }
